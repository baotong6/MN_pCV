...
In this work, we revisit the {\it Chandra} observations of the LW, employing a novel technique, known as the Gregory-Loredo period searching algorithm, to detect periodic signalshave taken full usage of \emph{Chandra} observations for LW. Meanwhile, the methods with more efficiency and the simulation with higher accuracy have been both operated. 
We explored the X-ray properties of 23 periodic sources (ten of them identified in \cite{2012ApJ...746..165H}). 
In Section~\ref{sec:obs}, we describes the preparation of the X-ray data observation we used and a raw list of X-raythe sources in the LW detection process. 
We then provide in Section~\ref{sec:methods} gives an brief  brief overview ofabout the period searching methods, with an emphasis on the Gregory-Loredo algorithm.main methods for period finding and focuses on the GL method we adopted in this work. 
Section~\ref{sec:results} is devoted to the confirmation of period searchingfinding results.
Section~\ref{sec:spectra} presents an X-ray the spectral analysis for the detectedse periodic sources. and the estimation of mass of WDs based on the Fe line diagnostic. 
...
1$^\dag$ & $0.51^{+0.75}_{-0.34}$ & 40 (fixed)  &-& 1.14/5  & $2.02^{+0.71}_{-0.57}$
\\
2 & $2.17^{+0.54}_{-0.46}$ & $20.7^{+56.9}_{-9.92}$ &$0.45^{+0.22}_{-0.17}$& 0.86/78  & $25.7^{+3.00}_{-2.24}$
\\
3 & $1.34^{+0.70}_{-0.40}$ & $46.4^{+46.1}_{-38.5}$ &-& 0.98/33  & $7.34^{+1.17}_{-0.86}$
\\
4 & $1.15^{+0.69}_{-0.50}$ & $11.3^{+53.3}_{-4.71}$ &-& 0.91/27  & $25.0^{+3.27}_{-4.91}$
\\
5 & $1.02^{+0.27}_{-0.23}$ & 40 (fixed) &-& 1.00/33 &  $6.21^{+0.86}_{-0.78}$
\\
...
\\
11 & $1.85^{+0.39}_{-0.33}$ & 40 (fixed)  &$0.29^{+0.12}_{-0.18}$& 1.22/47 &  $10.5^{+1.24}_{-1.14}$
\\
...
In addition to these two, for source \#2, the determined period is not only lower than 82 minutes (obtained from observation and simulation, \citet{2009MNRAS.397.2170G,2011ApJS..194...28K}), but also below the theoretical period minimum in the standard model, i.e., 65 minutes \citep{1982ApJ...254..616R}. Thus we consider it is more likely to be the spin period of an IP. While the miss of its orbital period detection could arisen from low inclination angle. 
Generally, the poor detection of spin period for IP candidates brings about the uncertainty for IP identification. It could mostly be resulted from the lower detection rate for short period, as demonstrated in Section \ref{subsec:simulation}.
