...
\begin{equation}
  N_{\rm obs,polar} = P_{\rm det} \times g \times \alpha_{\rm polar} \times N_{\rm tot},
\end{equation}
...
We further assume that the substantial sinusoidal variation occurs only in the ``two-pole'' situation, which needs the accretion of polar falls onto one pole and $i+\beta+\epsilon > 90^{\circ}$. 
Taking the equal probability hypothesis for parameters and at least half of polars with one-pole accretion, 
This allows us to take $\alpha_{\rm polar}$ = $50\%\times50\% = 25\%$ as the lower limit.
