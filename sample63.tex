...
Thus we restrict our analysis on three period ranges: (300, 3000), (3000, 10000) and (10000, 50000) sec, with a frequency resolution of $10^{-7}$, $10^{-8}$ and $10^{-9}$ {\bf Hz?}, respectively. 
...
The detection rate is essentially zero for net counts down to 100 and never exceeds 20\% for net counts of $\sim$150, based on the simulations presented in \cite{2012ApJ...746..165H}.
...
For each of the 10 sources, 100 simulated light curves are produced and fed to the GL algorithm.
Again, we take the 90\% probability threshold and an period accuracy of 0.1\% to define a valid detection. The percentage of valid detections $P_{det}$, as shown in column (9) in Table \ref{tab:src}. And $A_{M}$ was obtained with bin size fixed at 20. For directly comparison, we also ran the simulation based on the same $A_{0}$ defined in \citet{2012ApJ...746..165H} for the ten collective sources. A flagrant contrast was illustrated in Figure \ref{fig:com} based on that two columns. 
Clearly, GL method has better performance compared with LS method.
