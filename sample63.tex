...
While the GL algorithm does not rely on quantifying the local background, for comparison we plot in these panels the background level (yellow strip, the width of which represents 1\,$\sigma$ Poisso error) as determined in Section~\ref{subsec:detect}. 
{\bf [Give a try to describe these light curves.]}
...
is shown in the lower left panel of Figures~\ref{fig:pCV_sample_1} and \ref{fig:pCV_sample_2}. 
...
\end{minipage}
\caption{The phase-folded light curve at the two modulation periods, the long-term, inter-observation light curve and the spectra for \#1(\#14) source.
For middle panel, the valid detection (defined in \ref{subsec:detect}) are plotted with dots plus error range. While the non-valid detection are shown in arrows, providing only the upper limits. Meanwhile, the x-axis in right panel were plotted in discontinuous style for better demonstration effect, since the short interval over the last eight observations.}
...
\end{minipage}
\caption{Similar to Figure~\ref{fig:pCV_sample_2}, but for source 21/22.}
