...
The orbital period distribution of CVs is known to exhibit a minimum at about 70 minutes, a gap between 2--3 hours, and a maximum around 10 hours \citep{2003A&A...404..301R}. 
...
\hline
\hline
ID& R.A. & Decl. & Period & $P_{\rm GL}$ & H-ID & $C$ & $P_{det}$ & VI & Harmonics
\\
LW & $\circ$ & $\circ$ & (s) & & & & \% & & 
\\ 
...
\label{subsec:class}
While it is generally difficult to unambiguously identify the nature of an X-ray source without knowing its optical counterpart, the X-ray temporal and spectral properties of the 23 periodic sources contain important information that actually allow for a reasonable classification for many of them. 

First of all, all but one (\#25) of these sources are found in the 2--10 keV luminosity range of $10^{31}-10^{33}\rm~erg~s^{-1}$, which is typical of CVs. 
On the other hand, coronally active binaries (ABs), which are thought to dominate the number of detectable X-ray sources in the LW \citep{2009Natur.458.1142R}, generally have unabsorbed 2--10 keV luminosities below $\rm 10^{31}~erg~s^{-1}$ \citep{2006A&A...450..117S}. 
This stems from the empirical fact that the coronal X-ray luminosity of low-mass stars saturates at $\sim$10$^{-3}$ of the bolometric luminosity (reference).
...
Nevertheless, most of these sources do show a very hard continuum that are typical of CVs, although this could be partly owing to our selection for the raw source list (Section~\ref{subsec:detect}).
Further, the vast majority of the detected periods fall between 1--4 hours, which are also consistent with the known period range of CVs, as illustrated in Figure~\ref{fig:N_P}.
Therefore, the global X-ray properties point to CVs dominating the detected periodic sources, a conclusion also drawn by \citet{2012ApJ...746..165H}.

The most likely exception is source \#25. This source has an unabsorbed 2-10 keV luminosity of $\rm 7.5 \times 10^{30}~erg~s^{-1}$, the lowest among all 23 sources. Its X-ray spectra, characterized by a best-fit plasma temperature of 2.3 keV (notably with large uncertainties; Table~\ref{tab:spec}), is also the softest among all sources.
These features are more typical of ABs. 
Moreover, the phase-folded light curve suggests that the X-ray flux drops to the background level for nearly half of the 47317-sec period (Figure~\ref{fig:Figure_p}). 
This may be due to either a polar which produces hard X-rays from only one of its two poles, or a total eclipse lasting for half cycle.
In the latter case, the eclipsed star cannot be a WD, because of the mismatch between the stellar radius (order $10^4$ km)
and the orbital separation (order $10^7e$~km).
Rather, the eclipsed star is likely to have a size not much smaller than the orbital separation.  
The nearly no emission from one star hinted it could be an A-type star which does not produce significant X-rays due to the weakness of surface magnetic field (reference). Then the orbit radius would be around 0.02 AU, taking $3M_\odot$ as the total mass.  
...
For magnetic CVs, including IPs and polars, their orbit modulation could give rise to more complicated shape of light curve. For polars, consider the simplistic situation, whose accretion is nearly onto one pole, then the shape of light curve would be divided into two groups. Suppose an inclination $i$, the angle between rotation axis and magnetic axis is $\beta$, the magnetic colatitude of accretion is $\epsilon$. Then if $i+\beta+\epsilon > 90^{\circ}$, we would see alternately two poles, the shape would be like \#6, with half of orbit cycle showing no emission. This so-called "two-pole"  behavior are more complicated if the accretion goes to both two poles, the gap with half orbit cycle would be partially filled (like \#16) by second pole's emission or even making the light curve constant. Sometimes it may completely reverse in hard X-ray band cause the accretion shock in second pole producing most of hard X-rays \citep{1985A&A...148L..14H}. On the other hand, if $i+\beta+\epsilon < 90^{\circ}$ and $\beta < i$, the accreting pole would be always visible. In that case, the X-ray flux would be roughly constant, although sometimes some dips may been seen due to the obscuring of accretion stream.
...
Two of these periodic sources, \#1(14) and \#21(22) can be determined as they have two confirmed period.
...
%The characteristics of the phase-folded light curves provide important clues to the source origin.
...
%The choice of value is to balance the number of counts taken from observation and simulation. 
...
\end{equation}
$N_i$ is the product of number of sources detected in the bin. DR denotes the detection rate weighted from simulation. Then we sum all $Np_{i}$ together and force it equals to 20,  getting $\alpha$ about 16\%. The comparison between $Np_{i}$ and the real detected 20 periodic sources(two determined IPs and one AB excluded) are plotted in Figure \ref{fig:NP_sim}. The perfect agreement between their population proves the validity of estimation.
...
While for DNe, we are constrained by lack of reliable X-ray luminosity function(XLF). We have already set the three as the upper limit of DNe in Section \ref{subsec:class}. However, the steeper XLF for DNe than the XLF for total sample would only contribute the lower limit for their proportion. Though this conflict leads to the uncertainty of estimation,  we still present the estimation for fraction of DNe as supplement.
Intrinsically, the eclipse of white dwarf depends on the inclination angle, radius of companion star and orbit. If we set $i$ as inclination, $R_1$,$R_2$,$R_{orb}$ as radius of WD, companion star and orbit respectively. then eclipsing happens when :
\begin{equation}
{R_{orb}\times cos(90-i)}\leq { R_1+R_2}
\end{equation}
Assuming that inclination angle is even distributed in $(0,90^\circ)$, the probability of eclipsing(hereafter EP) are plotted in Figure \ref{fig:simpCV}. The value of $R_1, R_2$ and $R_{orb}$ are taken from simulation operated by \citep{2011ApJS..194...28K}, in which $R_1$ is fixed at when $M_{WD}$ equals 0.75 solar mass.
\begin{figure}
\centering
...
GL method employs piecewise function for periodic signal. Each model has $(m+2)$ parameters: angular frequency $\omega={2\pi/P}$, P represents period; phase parameter $\phi$; and m values $r_j$, signifies the counts rate in each phase bin where j=1 to m. The following we replace $H_i$ with $M_i$ to denote the model where $i$ represents the number of bins in piecewise model. Then the Bayes's theorem can be written as:
